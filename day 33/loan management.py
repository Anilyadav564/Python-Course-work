...
class Customer:
    def __init__(
        self, customer_id, name, email, phone_number, age, income, credit_score
    ):
    ...
    def check_eligibility(self):
        return not (
            self.age < 21
            or self.credit_score < 650
            or self.income < 25000
        )
...
class Loan(ABC):
    ...
    def display_statement(self):
        ...
        if not self.repayment_history:
            ...
            for index, payment in enumerate(self.repayment_history, start=1):
                print(f"Payment {index}: {payment}")
